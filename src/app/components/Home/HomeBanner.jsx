  ...
  return (
    <div className=' flex flex-col 2xl:flex-col lg:flex-row gap-5  justify-around '>
      {bannerArray.map((BannerProd) => (
        <Link to="" key={BannerProd.id} className='flex flex-col-reverse md:flex-row justify-between items-center rounded-lg bg-white grow p-6 '>
          <div className='flex flex-col  gap-10 w-[60%]  lg:w-[40%] justify-around text-center md:text-start'>
              <div className='flex flex-col gap-4 '>
                <h1 className='text-md md:text-2xl font-medium hover:text-(--accent-secondary) '>{BannerProd.title}</h1>
              ...
              <div>
                ...
                <div className='flex gap-2 items-center justify-center md:justify-start'>
                  <h3 className='text-md md:text-2xl font-medium'>${BannerProd.originalPrice}</h3>
          ...
          <div className=' h-full flex grow  justify-center items-center'>
            <img src={BannerProd.thumbnail} alt="" className='w-70 lg:w-30 lg:grow'/>
          </div>
        </Link>
      ))}
    </div>
